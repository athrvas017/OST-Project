...
int main(){
    ...
    int Waiting_Time[100], Turn_Around_Time[100], Exit_Time[100];
    ...
    while (completed_count < n){
        ...
        for (int i = 0; i < n; i++){
            if (Arrival_Time[i] <= current_time && !completed[i]){
                if (Burst_Time[i] < lowest_bt){
                    lowest_bt = Burst_Time[i];
                    idx = i;
                }
                else if (Burst_Time[i] == lowest_bt){
    ...
    for (int i = 0; i < n; i++){
        cout << "P" << Process_ID[i] << "\t" << Arrival_Time[i] << "\t"
             << Burst_Time[i] << "\t" << Exit_Time[i] << "\t" << Turn_Around_Time[i] << "\t" << Waiting_Time[i] << "\n";
    }
